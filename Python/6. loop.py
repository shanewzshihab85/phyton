...
products = {
    'product1': {'name': 'Laptop', 'stock': 5, 'threshold': 10},
    'product2': {'name': 'Smartphone', 'stock': 15, 'threshold': 20},
    'product3': {'name': 'Headphones', 'stock': 25, 'threshold': 5},
    'product4': {'name': 'Monitor', 'stock': 2, 'threshold': 5},
    'product5': {'name': 'Keyboard', 'stock': 8, 'threshold': 10}
}
...
def check_stock_levels(products):
    ...
    for product_id, product_info in products.items():
        ...
        if stock < threshold:
            alert = f"Restock Alert: {name} (Stock: {stock}, Threshold: {threshold})"
            restock_alerts.append(alert)
    
    return restock_alerts

alerts = check_stock_levels(products)

if alerts:
    for alert in alerts:
        print(alert)
else:
    print("No restock needed for any product.")
